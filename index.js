...
const requiredEnvVars = ['DISCORD_TOKEN', 'CLIENT_ID', 'DEEPGRAM_API_KEY', 'FIREWORK_API_KEY', 'SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY'];
...
client.on('interactionCreate', async (interaction) => {
    ...
    // Add a timeout to ensure we always respond to interactions
    const timeout = setTimeout(async () => {
        if (!interaction.replied && !interaction.deferred) {
            try {
                await interaction.reply({ 
                    content: 'This command is taking longer than expected. Please wait...', 
                    ephemeral: true 
                });
            } catch (error) {
                console.error('Timeout reply error:', error);
            }
        }
    }, 2500);
            ...
            try {
                ...
                recordings.forEach(recording => {
                    selectMenu.addOptions({
                        label: recording.title.length > 100 ? recording.title.substring(0, 97) + '...' : recording.title,
